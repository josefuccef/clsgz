...
async function fetchChannels() {
 ...
 try {
  ...
  writeDataToFile("./public/canalSport.json", extractedChannels);
 } catch (error) {
...
app.post("/update", (req, res) => {
 ...
 if (Array.isArray(updatedMatches)) {
  writeDataToFile("./public/matches.json", updatedMatches); // حفظ التحديثات في ملف matches.json
...
app.get("/", async (req, res) => {
 try {
  ...
  let matches = readDataFromFile("./public/matches.json");
  ...
  let channels = readDataFromFile("./public/Sport.json");

  if (!channels || channels.length === 0) {
   await fetchChannels(); // جلب القنوات
   channels = readDataFromFile("./public/Sport.json"); // إعادة قراءة البيانات بعد الجلب
  }
...
app.get("/edit", (req, res) => {
 const matches = readDataFromFile("./public/matches.json");
